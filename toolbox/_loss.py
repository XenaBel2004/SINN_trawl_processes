...
class StatLoss(_Loss, metaclass=ABCMeta):
    ...
    def __init__(self, target, pointwise_loss="mse_loss", device="cpu", **kwargs):
    ...
    def acf(x, lags=None, method="fft"):
        ...
        if method == "fft":
            ...
            acf = torch.fft.ifft(f * f.conj(), dim=0).real[: x.shape[0]].mean(axis=1)
    ...
    def gauss_kde(x, lower, upper, n, bw=None):
        ...
        return (
            torch.sum(
                torch.exp(-0.5 * torch.square((x[:, None] - grid[None, :]) / bw)),
                axis=0,
            )
            / norm_factor
        )
...
class DensityLoss(StatLoss):
    ...
    def forward(self, input):
        ...
        input: tensor of any shape
            The input trajectory as generated by an NN.
        """
        _input = self.gauss_kde(input, lower=self.lower, upper=self.upper, n=self.n)
        return self._loss(_input, self._target)
