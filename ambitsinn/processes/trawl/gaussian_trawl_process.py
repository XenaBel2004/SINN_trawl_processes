# -*- coding: ascii -*-
r"""Concrete implementation of a *Gaussian* trawl process.
...
class GaussianTrawlProcess(TrawlProcess):
    """Gaussian Trawl Process class.

    Gaussian trawl process are trwal process where the Levy seed ``L'`` is
    ...
    **default_opts
    ...
    def __init__(
        ...
        **default_opts,
        ...
        def seed_cumulant(theta: Tensor) -> Tensor:
            r"""Log-characteristic function (cumulant) of the Gaussian Levy seed.
            ...
            theta_c = theta.to(
                dtype=torch.complex64 if theta.dtype in (torch.float32, torch.complex64) else torch.complex128
            )
    ...
    def pdf(self, x: Tensor) -> Tensor:
        ...
        """
        return Normal(self.mean, self.std).log_prob(x).exp()

    def at_times(self, times: Tensor, rng: Optional[Generator] = None) -> "GaussianTrawlProcessFDD":
...
class GaussianTrawlProcessFDD(TrawlProcessFDD):
    ...
    Implements the slice sampling from gaussian-seed required by the base
    class :class:`TrawlProcessFDD`.
    """
    ...
    def __init__(
        ...
        """Initialize GaussianTrawlProcessFDD object.
